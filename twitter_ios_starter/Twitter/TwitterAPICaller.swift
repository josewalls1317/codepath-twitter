...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func unretweet(tweetId:Int, succes: @escaping () -> (), failure: @escaping (Error) -> ()) {
        
        let url = "https://api.twitter.com/1.1/statuses/unretweet/\(tweetId).json"
        TwitterAPICaller.client?.post(url, parameters: ["id":tweetId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in succes()
        }, failure: { (tasl: URLSessionDataTask?, error: Error) in
            failure(error)
        })
    }
